...
@Service
public class TallyServiceExtrinsicImpl implements TallyExtrinsicService {
    ...
    @Resource
    private TallyExtrinsicMapper tallyExtrinsicMapper;
...
//        Integer flag = tallyExtrinsicMapper.getIsDept(securityUtils.getLoginUserId());
//        searchDTO.setFlag(flag);
//        searchDTO.setLoginId(securityUtils.getLoginUserId() + "");
        List<TPrdWorkPlanDTO> workPlanList = tallyExtrinsicMapper.getWorkPlan(searchDTO);
        List<Map<String, Object>> maps = tallyExtrinsicMapper.getWorkPlanCargoInfo(searchDTO);
        Iterator<TPrdWorkPlanDTO> iter = workPlanList.iterator();
        while (iter.hasNext()) {
            TPrdWorkPlanDTO po = iter.next();
            //查询该计划的所有货物的作业方式
//            List<Map<String, Object>> list = tallyExtrinsicMapper.getWorkType(po.getTrustId());
...
//            }
        ...
        if (!CollectionUtils.isEmpty(workPlanList)) {
            for (TPrdWorkPlanDTO prdWorkPlanDTO : workPlanList) {
                ...
                if ("1".equals(prdWorkPlanDTO.getIsDirectAccess())) {
                    ...
                    List<Map<String, Object>> tBusCargoInfoDTOS = tallyExtrinsicMapper.getTrustInfoNo(prdWorkPlanDTO.getTrustId().toString(), type);
        ...
        for (TPrdWorkPlanDTO dto : workPlanList) {
            if ("2".equals(dto.getPlanType())) {
                //集疏港计划显示船名
                List<Map<String, Object>> list = tallyExtrinsicMapper.getShipName(dto.getTrustId());
    ...
    public List<Map<String, Object>> getProcessInfoList(String processCode) {
        ...
        return tallyExtrinsicMapper.getProcessInfoList(processCode);
    }

    @Override
    public List<Map<String, Object>> getMechanics(TPrdDispatchExtrinsicDTO tPrdDispatchSecondarySearchDTO) {
        return tallyExtrinsicMapper.getMechanics(tPrdDispatchSecondarySearchDTO);
    }

    @Override
    public List<Map<String, Object>> getStorageList(TPrdDispatchExtrinsicDTO tPrdDispatchSecondarySearchDTO) {
        return tallyExtrinsicMapper.getStorageList(tPrdDispatchSecondarySearchDTO);
    }
...
//        tPrdDispatchSecondarySearchDTO.setOperatorsId(securityUtils.getLoginUserId());
        return tallyExtrinsicMapper.getTransfer(tPrdDispatchSecondarySearchDTO);
    }

    @Override
    public List<Map<String, Object>> getCarFer(TPrdDispatchExtrinsicDTO tPrdDispatchSecondarySearchDTO) {
        //判断作业过程
        Map<String, Object> process = tallyExtrinsicMapper.getIsZq(tPrdDispatchSecondarySearchDTO.getProcessCode());
        //指令票货ID
        String planCode = "";
        if ("1".equals(process.get("isDirectAccess").toString())) {
            ...
            planCode = tallyExtrinsicMapper.getPlanCodeZ(tPrdDispatchSecondarySearchDTO);
        } else {
            //根据指令ID查询指令票货ID
            planCode = tallyExtrinsicMapper.getPlanCode(tPrdDispatchSecondarySearchDTO);
        }
        if (planCode != null && (!"".equals(planCode))) {
            String ids = "(" + planCode + ")";
            //新地磅车号
            List<Map<String, Object>> list = tallyExtrinsicMapper.getCarFerNew(ids);
    ...
    public List<Map<String, Object>> getTruckNo(TPrdDispatchExtrinsicDTO tPrdDispatchSecondarySearchDTO) {
        ...
        if (planCode != null && (!"".equals(planCode))) {
            String ids = "(" + planCode + ")";
            //新地磅车号
            List<Map<String, Object>> list = tallyExtrinsicMapper.getCarFerNewEx(ids);
    ...
    public List<Map<String, Object>> getCargoInfoId(String trustId, String planId, String processCode) {
        List<Map<String, Object>> cargoInfoIdList = tallyExtrinsicMapper.getCargoInfoId(trustId, planId);
        if (processCode != null && (!"".equals(processCode))) {
            //判断作业过程
            Map<String, Object> process = tallyExtrinsicMapper.getIsZq(processCode);
            //直取
            if ("1".equals(process.get("isDirectAccess").toString())) {
                //直取(根据票货ID查询指令票货ID)
                List<String> type;
                ...
                for (Map<String, Object> map : cargoInfoIdList) {
                    List<Map<String, Object>> businessNoList = tallyExtrinsicMapper.getTrustId(Long.parseLong(map.get("id").toString()), type);
    ...
    public List<Map<String, Object>> getCargoInfoIdTr(String planId) {
        return tallyExtrinsicMapper.getCargoInfoIdTr(planId);
    }

    public void insertIntOutYard(TYardTallyItemPO po, TYardTallyPO tYardTallyPO) {
        //查询出入库标识
        String inOutType = tallyExtrinsicMapper.getInoutType(tYardTallyPO.getProcessCode());
    ...
    public List<Map<String, Object>> getStore(Long workPlanId) {
        return tallyExtrinsicMapper.getStore(workPlanId);
    }

    @Override
    public List<Map<String, Object>> getStackPosition(Long id) {
        return tallyExtrinsicMapper.getStackPosition(id);
    }
    ...
    public List<TYardTallyPO> getWaitWork(Long trustId,String macName) {
        ...
        List<TYardTallyPO> list = tallyExtrinsicMapper.getWaitWork(trustId,macName);
        LOGGER.info("查询数据成功");
    ...
    public Pages<TYardTallyItemPO> getTallyRecord(TallyRecordSearchDTO tallyRecordSearchDTO, PageParameter pageParameter) {
        ...
        Pages<TYardTallyItemPO> pages = PageHelperUtils.limit(pageParameter, () -> {
            return tallyExtrinsicMapper.getTallyRecord(tallyRecordSearchDTO);
        });
        LOGGER.info("查询数据成功");
        LOGGER.exit(methodName, "业务数据同步服务[end]");
        return pages;
    ...
    public TYardTallyPO getTallyInfoById(TallyRecordSearchDTO tallyRecordSearchDTO, PageParameter pageParameter) {
        return tallyExtrinsicMapper.getTallyInfoById(tallyRecordSearchDTO);
    }

    @Override
    public TYardTallyPO getTallyNew(TallyRecordSearchDTO tallyRecordSearchDTO, PageParameter pageParameter) {
        return tallyExtrinsicMapper.getTallyNew(tallyRecordSearchDTO);
    }
    ...
    public Map<String, Object> getTallyRecordSum(TallyRecordSearchDTO tallyRecordSearchDTO) {
        ...
        Map<String, Object> map = tallyExtrinsicMapper.getTallyRecordSum(tallyRecordSearchDTO);
        LOGGER.info("查询数据成功");
        LOGGER.exit(methodName, "业务数据同步服务[end]");
        return map;
    }
    ...
    public List<AppTallyLadingDTO> getCarDetailedList(TYardMeasureSearchDTO tYardMeasureSearchDTO) {
        ...
        //根据指令查票货
        String str = tallyExtrinsicMapper.getTrustCargoId(tYardMeasureSearchDTO.getTrustId());
        if (str == null) {
        ...
        if (switchIsFrontier) {
            ...
            //校验根据作业过程进行校验
            String isFrontier = tallyExtrinsicMapper.getProcessIsFrontier(tYardMeasureSearchDTO.getProcessCode());
        ...
        List<AppTallyLadingDTO> carDetailedList = tallyExtrinsicMapper.getCarDetailedList(tYardMeasureSearchDTO);
        LOGGER.info("查询数据成功");
        LOGGER.exit(methodName, "业务数据同步服务[end]");
...
//            return tallyExtrinsicMapper.getNumber(appTallyCoilNumDTO);
//        });
//        LOGGER.info("查询数据成功");
//        LOGGER.exit(methodName, "业务数据同步服务[end]");
//        return pages;
//    }
    ...
    public List<DepartureDTO> getDepartureList(String truckPlate) {
        ...
        LOGGER.info("查询数据成功");
        LOGGER.exit(methodName, "业务数据同步服务[end]");
        List<DepartureDTO> departureList = tallyExtrinsicMapper.getDepartureList(truckPlate);
    ...
    public List<DepartureDTO> getDepartureRecordList(String startDate, String endDate) {
        ...
        LOGGER.info("查询数据成功");
        LOGGER.exit(methodName, "业务数据同步服务[end]");
        List<DepartureDTO> departureList = tallyExtrinsicMapper.getDepartureRecordList(startDate, endDate);
        return departureList;
    }

    @Override
    public String selectBh(String code) {
        return tallyExtrinsicMapper.selectBh(code);
    }

    @Override
    public List<Map<String, Object>> getHatch(Long id) {
        return tallyExtrinsicMapper.getHatch(id);
    }

    @Override
    public List<TallyCargoDTO> getCargoStatistics(List<Long> ids) {
        List<TallyCargoDTO> cargoStatistics = tallyExtrinsicMapper.getCargoStatistics(ids);
    ...
    public void updateTally(TYardTallyPO tYardTallyPO) {
        ...
        //获取理货记录详情信息
        TYardTallyPO tallyInfoById = tallyExtrinsicMapper.getTallyInfoById(tallyRecordSearchDTO);
        //根据理货记录子作业过程查询主作业过程的源和目的
        Map<String, Object> process = tallyExtrinsicMapper.getProcess(tallyInfoById.getProcessCode());
        ...
        if (flag1 || flag2 || flag3 && (!"10210004".equals(tallyInfoById.getProcessCode())) && (!"10200005".equals(tallyInfoById.getProcessCode()))) {
            ...
            Integer isPre = tallyExtrinsicMapper.getIsPre(tallyInfoById.getProcessCode());
            if (isPre == null) {
        ...
        if (tYardTallyPO.getCoilList() != null && tYardTallyPO.getCoilList().size() != 0) {
            //先处理一下之前的数据
            //查询钢号信息
            List<AppTallyCoilNumDTO> numDTOList = tallyExtrinsicMapper.getCoilNumIdList(tYardTallyPO.getId().toString());
            if (numDTOList != null && numDTOList.size() != 0) {
                //删除理货钢号关系表
                tallyExtrinsicMapper.deleteCoilNum(tYardTallyPO.getId().toString());
                //有钢号信息。进行处理
                tallyExtrinsicMapper.updateCoilNum(numDTOList);
            }
            saveCoilList(tYardTallyPO.getCoilList(), tYardTallyPO);
            //修改磅单表备注信息
            String weightId = tallyExtrinsicMapper.getWeightId(tYardTallyPO.getId());
            if (StringUtils.isNotEmpty(weightId)) {
                ...
                result = "标号:" + result;
                tallyExtrinsicMapper.updateWeightRemark(weightId, result);
            }
        }
    }
    ...
    public void insertMacTally(TYardTallyMacPO tYardTallyMacPO) {
        TPrdWorkPlanPO workPlanInfo = tallyExtrinsicMapper.getWorkPlanInfo(tYardTallyMacPO.getPlanId());
        ...
        //判断是开始还是结束,1开始2结束
        if ("1".equals(tYardTallyMacPO.getStatus())) {
            if("1025".equals(workPlanInfo.getProcessCode())){
                ...
                //装船
//                List<TYardTallyPO> waitWorkList = tallyExtrinsicMapper.getWaitWork(workPlanInfo.getTrustId(),tYardTallyMacPO.getMacName());
//                if(CollectionUtils.isEmpty(waitWorkList)){
//                    throw new BusinessRuntimeException("获取后场转运信息失败");
//                }
//                tYardTallyMacPO.setTallyId(waitWorkList.get(0).getId());
            }
            //查询该门机下有没有未理货的
            List<String> unTallyList = tallyExtrinsicMapper.getUnTallyList(tYardTallyMacPO);
            ...
            tallyExtrinsicMapper.insertMacTally(tYardTallyMacPO);
        }else {
            ...
            tallyExtrinsicMapper.updateMacTally(tYardTallyMacPO);
        }
    }

    @Override
    public List<Map<String, Object>> getDept() {
        return tallyExtrinsicMapper.getDept();
    }
    ...
    public void deleteNotes(TYardTallyPO tYardTallyPO) {
        ...
        if (flag1 || flag2 || flag3 && (!"10210004".equals(tallyInfoById.getProcessCode())) && (!"10200005".equals(tallyInfoById.getProcessCode()))) {
            ...
            if (isPre == 1) {
                ...
                if (tallyInfoById.getRelationId() == null) {
                    ...
                    po.setId(tallyId);
                    tallyExtrinsicMapper.deleteNotes(po);
                    //删除理货记录(子表)
                    tallyExtrinsicMapper.deleteNotesItem(tallyId);
                    //根据理货ID获取港存明细ID
                    List<Long> ids = tallyExtrinsicMapper.getPortStorageDetailId(tallyId);
        ...
        //查询钢号信息
        List<AppTallyCoilNumDTO> numDTOList = tallyExtrinsicMapper.getCoilNumIdList(tYardTallyPO.getTallyId());
        if (numDTOList != null && numDTOList.size() != 0) {
            //删除理货钢号关系表
            tallyExtrinsicMapper.deleteCoilNum(tYardTallyPO.getTallyId());
            //有钢号信息。进行处理
            tallyExtrinsicMapper.updateCoilNum(numDTOList);
        }
    }
    ...
    public void delete(Long tallyId) {
        ...
        tallyExtrinsicMapper.deleteNotesItem(tallyId);
        //更新关联ID
        tallyExtrinsicMapper.updateNotesRelationId(tallyId);
        //根据理货ID获取港存明细ID
        List<Long> ids = tallyExtrinsicMapper.getPortStorageDetailId(tallyId);
        if (ids != null && ids.size() != 0) {
            //删除港存
    ...
    public void deleteNoReshipment(Long tallyId, TYardTallyPO tallyInfoById) {
        ...
        if (tallyInfoById.getWeighbridgeId() != null) {
            if (tallyInfoById.getWorkErWeiId() == null) {
                ...
                //新地磅
                String status = tallyExtrinsicMapper.getWeightStatusNew(tallyInfoById.getWeighbridgeId().toString());
                if (status == null) {
                    throw new BusinessRuntimeException("未查询到车辆信息,不允许删除!");
                }
                if ("1".equals(status)) {
                ...
                tallyExtrinsicMapper.updateWeightStatusNew(tallyInfoById.getWeighbridgeId().toString());
            }
        }
    ...
    public void updateTallyInfo(TYardTallyPO tYardTallyPO) {
        ...
        //修改理货记录主表
        tallyExtrinsicMapper.updateTally(tYardTallyPO);
        //判断作业过程的源和目的是否为场地，是的场地垛号必填
        Map<String, Object> process = tallyExtrinsicMapper.getIsZq(tYardTallyPO.getProcessCode());
...
//                insertMiss(po);
                if (po.getCargoCode() != null) {
                    Integer isTally = tallyExtrinsicMapper.getIsTally(po.getCargoCode());
                    if (isTally != null && isTally == 1) {
                        //根据理货ID获取港存明细ID
                        List<Long> ids = tallyExtrinsicMapper.getPortStorageDetailItemId(po.getId());
                        ...
                        if ("10200005".equals(tYardTallyPO.getProcessCode()) || "10210004".equals(tYardTallyPO.getProcessCode())) {
                            ...
                            TYardTallyItemPO pos = tallyExtrinsicMapper.getStorage(po.getId());
            ...
            //修改理货记录子表
            tallyExtrinsicMapper.updateTallyItem(po);
        }
        LOGGER.info("修改成功");
        LOGGER.exit(methodName, "业务数据同步服务[end]");
    }
    ...
    public void tally(TYardTallyPO tYardTallyPO) {
        ...
        if (SourceTargetTypeEnum._01.getCode().equals(tYardTallyPO.getSource()) && SourceTargetTypeEnum._05.getCode().equals(tYardTallyPO.getDestination())) {
            ...
            if (tYardTallyPO.getRelationId() != null) {
                //查询是否已经理货了
                Integer count = tallyExtrinsicMapper.getRelationIdHc(tYardTallyPO);
                if (count > 0) {
                    throw new BusinessRuntimeException("该数据已理货，不能重复理货");
                }
                //关联相关理货ID
                tallyExtrinsicMapper.updateRelationId(tYardTallyPO);
            }
        }
        //没有前置要把relationId置为空
        if (!tYardTallyPO.getIsFrontType()) {
            //指令 - 车牌号 - 作业过程 查询关联ID，理完才可继续理货
            Integer count = tallyExtrinsicMapper.getRelationIdQy(tYardTallyPO);
            if (count > 0) {
        ...
        //新增主表信息
        int i = tallyExtrinsicMapper.tally(tYardTallyPO);
        if (i != 0) {
...
//                }
            ...
            //添加子表信息
            tallyExtrinsicMapper.tallyItem(listTallyList);
            //附件保存
    ...
    public Map<String, Object> getEqInfo(Long planId, String equipmentNo, String type) {
        ...
        if ("1".equals(type)) {
            mechanics = tallyExtrinsicMapper.getMechanics(tPrdDispatchSecondarySearchDTO);
        } else {
            mechanics = tallyExtrinsicMapper.getTransfer(tPrdDispatchSecondarySearchDTO);
        }
    ...
    public void saveCoilList(List<AppTallyCoilNumDTO> list, TYardTallyPO tYardTallyPO) {
        ...
        tallyExtrinsicMapper.updateCoilList(list);
        //理货标号关系表
        tallyExtrinsicMapper.insertCoilList(list);
    }
...
//            //存入垛位
//            tallyExtrinsicMapper.insertStackPosition(map);
//    }
    public void insertMiss(TYardTallyItemPO po) {
        //查询该场地下是否有该输入垛号名称
        List<Long> list = tallyExtrinsicMapper.getIsStackPosition(po);
        if (list != null && list.size() != 0) {
    ...
    public void tallyCheChuan(TYardTallyPO tYardTallyPO) {
        ...
        //根据磅单ID获取磅单数据
        TYardTallyPO weightInfoPO = tallyExtrinsicMapper.getWeightInfo(tYardTallyPO);
        ...
        //验重
        int count = tallyExtrinsicMapper.getIsBdTally(tYardTallyPO);
        if (count > 0) {
...
//            sysFileService.saveFileBusRelation(fileIds, id);
    ...
    public void updateWeight(TYardTallyPO tYardTallyPO) {
        tallyExtrinsicMapper.updateWeight(tYardTallyPO);
    }

    public void updateWeightNew(TYardTallyPO tYardTallyPO) {
        int i = tallyExtrinsicMapper.updateWeightNew(tYardTallyPO);
        if (i == 0) {
    ...
    public void updateWeightStatus(String id) {
        tallyExtrinsicMapper.updateWeightStatus(id);
    }

    @DS("simeauto")
    @Transactional(isolation = Isolation.READ_COMMITTED, rollbackFor = Exception.class, propagation = Propagation.REQUIRES_NEW)
    public String getWeightStatus(String id) {
        return tallyExtrinsicMapper.getWeightStatus(id);
    }
...
//            }
    ...
    public Map<String, Object> getWorkProgress(Long trustId, String workDate, String classCode, String cargoInfoNo) {
        ...
        // 查询用于统计的作业过程
        List<MWorkProcessPO> workProcessList = tallyExtrinsicMapper.getMWorkProcessList();

        // 查询作业通知单计划量
        Map<String, Object> trustPlanMap = tallyExtrinsicMapper.getTrustPlan(trustId);
        resultMap.put("trustPlan", trustPlanMap);

        // 查询票货信息
        List<Map<String, Object>> cargoInfoList = tallyExtrinsicMapper.getCargoInfoList(trustId, cargoInfoNo);
        for (Map<String, Object> cargoInfo : cargoInfoList) {
            Long cargoInfoId = Long.parseLong(cargoInfo.get("cargoInfoId").toString());

            // 查询集、疏港
            Map<String, Object> inOutPortWorkData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
            ...
            // 查询直取装、卸船
            Map<String, Object> zqWorkData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                    workProcessList.stream()
            ...
            // 查询倒运装、卸船
            Map<String, Object> dyWorkData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                    workProcessList.stream()
            ...
            // 查询落地装船
            Map<String, Object> luoWorkData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                    workProcessList.stream()
            ...
            // 查询车-岸前沿落地集港
            Map<String, Object> caWorkData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                    workProcessList.stream()
            ...
            if (StringUtils.isNotBlank(workDate) && StringUtils.isNotBlank(classCode)) {
                // 查询本班作业量-集、疏港
                Map<String, Object> inOutPortWorkTimeData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                        workProcessList.stream()
                                .filter((MWorkProcessPO po) -> "10".equals(po.getTallyDataStat()))
                ...
                // 查询本班作业量-直取装、卸船
                Map<String, Object> zqWorkTimeData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                        workProcessList.stream()
                                .filter((MWorkProcessPO po) -> "20".equals(po.getTallyDataStat()))
                                .collect(Collectors.toList()).stream().map(MWorkProcessPO::getProcessCd).collect(Collectors.toList()), workDate, classCode);
                ...
                // 查询本班作业量-倒运装、卸船
                Map<String, Object> dyWorkTimeData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                        workProcessList.stream()
                                .filter((MWorkProcessPO po) -> "30".equals(po.getTallyDataStat()))
                                .collect(Collectors.toList()).stream().map(MWorkProcessPO::getProcessCd).collect(Collectors.toList()), workDate, classCode);
                ...
                // 查询本班作业量-落地装船
                Map<String, Object> luoWorkTimeData = tallyExtrinsicMapper.getProgressWorkData(cargoInfoId,
                        workProcessList.stream()
                                .filter((MWorkProcessPO po) -> "40".equals(po.getTallyDataStat()))
                                .collect(Collectors.toList()).stream().map(MWorkProcessPO::getProcessCd).collect(Collectors.toList()), workDate, classCode);
    ...
    public Map<String, Object> getTransportEquipmentCargoInfo(Long trustId, String vehicleNo) {
        ...
        Map<String, Object> resultMap = tallyExtrinsicMapper.getTransportEquipmentCargoInfo(trustId, vehicleNo);
    ...
    public Map<String, Object> getTransportEquipmentCargoInfoZ(Long trustId, String vehicleNo) {
        ...
        LOGGER.enter(methodName, "业务执行");
        Map<String, Object> resultMap = tallyExtrinsicMapper.getTransportEquipmentCargoInfoZ(trustId, vehicleNo);
...
//        if (departureDTO.getWorkErweiId() != null) {
        ...
        List<Long> idList = tallyExtrinsicMapper.getNoteTally(departureDTO.getNoteId());
        if (idList != null && idList.size() != 0) {
            for (Long id : idList) {
                TYardTallyPO po = new TYardTallyPO();
                po.setId(id);
                //删除理货记录主表
                int i = tallyExtrinsicMapper.deleteNotes(po);
                if (i != 0) {
                    //删除理货记录(子表)
                    tallyExtrinsicMapper.deleteNotesItem(id);
                    //根据理货ID获取港存明细ID
                    List<Long> ids = tallyExtrinsicMapper.getPortStorageDetailId(id);
                    if (ids != null && ids.size() != 0) {
                        //删除港存
                        businessCommonService.deletePortStorageDetail(ids);
                    }
        ...
        tallyExtrinsicMapper.tallyDepartureSub(pos);
        ...
        //设置票货信息
        List<Map<String, Object>> map = tallyExtrinsicMapper.getCargoDeparture(departureDTO.getPlanNo());
        ...
        tallyExtrinsicMapper.tallyItem(list);
        //更新磅单理货状态
        tallyExtrinsicMapper.updateDeparture(departureDTO.getNoteId());
//        } else {
...
//        }
    ...
    public void departureRevoke(DepartureDTO departureDTO) {
        ...
        tallyExtrinsicMapper.deleteNotes(tYardTallyPO);
        int i = tallyExtrinsicMapper.deleteNotesItem(tYardTallyPO.getId());
        if (i != 0) {
            //根据磅单ID查询磅单状态
            TYardTallyPO weightInfo = tallyExtrinsicMapper.getWeightInfo(tYardTallyPO);
            if (weightInfo == null) {
                ...
                tallyExtrinsicMapper.updateWeightRevoke(tYardTallyPO);
            }
        } else {
    ...
    public void operateWeight(DepartureDTO departureDTO) {
        //老地磅
        tallyExtrinsicMapper.updateWeightInfo(departureDTO);
        tallyExtrinsicMapper.insertEmptyTruck(departureDTO);
    }

    @Override
    public List<Map<String, Object>> listDisShipVoyageApp() {
        return tallyExtrinsicMapper.listDisShipVoyageApp();
    }

    @Override
    public void tallyConfirm(TYardTallyPO tYardTallyPO) {
        TPrdWorkPlanPO workPlanInfo = tallyExtrinsicMapper.getWorkPlanInfo(tYardTallyPO.getPlanId());
        if (workPlanInfo == null) {
...
//        Map<String, Object> process = tallyExtrinsicMapper.getProcess(tYardTallyPO.getProcessCode());
//        if (process == null) {
//            throw new BusinessRuntimeException("获取主作业过程信息失败");
//        }
//        Map<String, Object> processDetailInfo = tallyExtrinsicMapper.getIsFrontType(tYardTallyPO.getProcessCode());
...
//        }
    ...
    public TYardTallyMacPO getNewDoorById(String planId, String macName) {
        return tallyExtrinsicMapper.getNewDoorById(planId,macName);
    }
}
