...
import com.yy.ppm.equipment.mapper.EMaterialOutApplicationAppMapper;
...
@Service
public class EMaterialOutApplicationAppServiceImpl implements EMaterialOutApplicationAppService {

    @Resource
    private EMaterialOutApplicationAppMapper mapper;
     ...
     */
    @Override
    public Pages<EMaterialOutApplicationDTO> getList(EMaterialOutApplicationSearchDTO searchDTO, PageParameter parameter) {
        searchDTO.setCreateBy(securityUtils.getLoginUserId());
        return PageHelperUtils.limit(parameter, () -> {
            Page<EMaterialOutApplicationDTO> page = mapper.selectList(searchDTO);
            return page;
        });
    }
    ...
    public void save(EMaterialOutApplicationDTO dto) {
        ...
        if (dto.getId() == null) {
            ...
            if (dto.getWarehouseOutTitle() == null || dto.getWarehouseOutTitle().trim().isEmpty()) {
                throw new BusinessRuntimeException("出库主题不能为空");
            }
            // 验证仓库
