 ...
 * @author system
 */
public interface EMaterialStockMapper {

    /**
     * 查询物资库存列表（分页）
     */
    Page<EMaterialStockDTO> selectStockList(EMaterialStockSearchDTO searchDTO);
    ...
    /**
     * 查询指定仓库的所有物资库存（用于创建盘点单）
     */
    List<EMaterialStockDTO> selectStockListByWarehouseId(@Param("warehouseId") Long warehouseId);

    /**
     * 查询物资库存列表（导出，分页）
     */
    Page<EMaterialStockDTO> selectStockListForExport(EMaterialStockSearchDTO searchDTO);
}
