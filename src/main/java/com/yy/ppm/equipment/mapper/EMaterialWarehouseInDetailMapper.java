 ...
 * @author system
 */
public interface EMaterialWarehouseInDetailMapper {
    ...
    /**
     * 查询物资最近一次入库的含税单价（用于盘点入库）
     * @param materialId 物资ID
     * @param warehouseId 仓库ID
     * @return 含税单价，如果没有找到则返回null
     */
    java.math.BigDecimal selectLatestTaxIncludedUnitPrice(
            @Param("materialId") Long materialId,
            @Param("warehouseId") Long warehouseId
    );

    /**
     * 根据仓库ID查询所有入库明细（用于盘点，不合并，每个入库明细一条记录）
     * @param warehouseId 仓库ID
     * @return 入库明细列表
     */
    List<EMaterialWarehouseInDetailDTO> selectListByWarehouseId(@Param("warehouseId") Long warehouseId);
}
